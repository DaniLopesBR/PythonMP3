...
def alternar_reproducao():
    ...
    if pygame.mixer.music.get_busy():
        ...
        # Alterar a cor do botão para indicar que a música está pausada
        button_pause_play.config(bg="#001dff")
...
def play_previous(current_song_label):
    global musica_atual, resultados, button_repeat, button_repeat_all, button_random
    ...
    # Alterar a cor dos botões de repetir e aleatório para indicar que estão sendo utilizados
    button_repeat.config(bg="#001dff")
    button_repeat_all.config(bg="#001dff")
    button_random.config(bg="#001dff")
...
def repetir_musica():
    global button_repeat
    if pygame.mixer.music.get_busy():
        reproduzir_musica(resultados[musica_atual], current_song_label)

        # Alterar a cor do botão para indicar que está sendo utilizado
        button_repeat.config(bg="#001dff")

# Função para repetir todas as músicas
def repetir_todas():
    global button_repeat_all
    if pygame.mixer.music.get_busy():
        reproduzir_musica(resultados[musica_atual], current_song_label)
...
# Função para reprodução aleatória
...
def main():
    ...
    global button_repeat, button_repeat_all, button_random  # Variáveis globais para os botões de repetir e aleatório
    ...
    root.geometry("210x380")  # Define as dimensões da janela
    ...
    # Botões para repetir e reprodução aleatória
    button_repeat = tk.Button(root, text="Repeat", command=repetir_musica, bg="#b2dafb", fg="#000000")
    button_repeat_all = tk.Button(root, text="Repeat All", command=repetir_todas, bg="#b2dafb", fg="#000000")
    button_random = tk.Button(root, text="Random", command=reproducao_aleatoria, bg="#b2dafb", fg="#000000")
   ...
   # Posicionar os botões na interface
    button_folder.grid(row=7, column=0)  # Botão para selecionar a pasta de músicas
    button_pause_play.grid(row=8, column=1)  # Botão para pausar ou retomar a reprodução da música atual
    button_previous.grid(row=8, column=0)  # Botão para reproduzir a música anterior na lista
    button_play.grid(row=7, column=1)  # Botão para reproduzir a música selecionada na lista
    button_next.grid(row=8, column=2)  # Botão para reproduzir a próxima música na lista
    button_stop.grid(row=9, column=1)  # Botão para parar a reprodução da música atual
    volume_slider.grid(row=1, column=4, rowspan=5, padx=5, pady=5)  # Slider para ajustar o volume

    # Posicionar botões de repetir e reprodução aleatória
    button_repeat.grid(row=9, column=0)
    button_repeat_all.grid(row=9, column=2)
    button_random.grid(row=10, column=1)
