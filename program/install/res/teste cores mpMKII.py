...
def reproduzir_musica(musica, current_song_label):
    ...
    current_song_label.config(text=os.path.basename(musica))
...
def play_selected(resultados, current_song_label):
    ...
    if selected_index:
        ...
        reproduzir_musica(selecionado, current_song_label)
        current_song_label.config(text=os.path.basename(selecionado))
...
def play_previous():
    ...
    if repetir_todas_var.get() or (repetir_musica_var.get() and musica_atual == 0):
        musica_atual = (musica_atual + len(resultados) - 1) % len(resultados)
    elif reproducao_aleatoria_var.get():
        musica_atual = randint(0, len(resultados) - 1)
    elif musica_atual > 0:
        musica_atual -= 1
...
def main():
    ...
    root.geometry("287x372")

    current_song_label = tk.Label(root, text="", bg="#b2dafb", fg="#000000")
    current_song_label.grid(row=5, column=3, columnspan=4)
...
def salvar_configuracao(pasta_musica):
    ...
    # Salvar a pasta selecionada no arquivo de configuração
    with open(config_file_path, "w") as f:
        json.dump({"last_folder": pasta_musica}, f)
...
def carregar_configuracao():
    ...
    if os.path.exists("config.json"):
        with open("config.json", "r") as f:
            config = json.load(f)
            pasta_musica = config.get("last_folder", "")
    return pasta_musica
