...
def repetir_todas():
    ...
    pygame.mixer.stop()  # Parar a reprodução atual
    musica_atual = 0  # Reset the current song index to the beginning of the list
...
def repetir_todas(root):
    ...
    repetir_todas_loop = True  # Set the repetir_todas loop control variable to True
    reproduzir_musica(resultados[musica_atual], current_song_label)  # Reproduz a música atual
    pygame.mixer.music.play(-1)  # Reproduz a lista de músicas em loop infinito
...
def check_music_status(root):
    if pygame.mixer.music.get_busy():
        ...
        on_music_end()  # If the music is not playing, call the on_music_end function

# Move the repetir_todas(root) line inside the main function
# ... (rest of your code)

# Initialize the repetir_todas loop control variable to False
repetir_todas_loop = False

# Pass the root variable to the repetir_todas function
repetir_todas(root)
